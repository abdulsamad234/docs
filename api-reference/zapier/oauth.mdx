...
description: "Complete guide to integrating with Push Lap Growth via Zapier using OAuth 2.0 authentication flow"
---
...
```
https://pushlapgrowth.com/connect/zapier/choose-affiliate-program
```
...
1. Log in to their Push Lap Growth account (if not already authenticated)
...
```
POST https://pushlapgrowth.com/api/zapier/token
```
...
- Users can revoke access through their Push Lap Growth dashboard
...
2. Zapier redirects to `https://pushlapgrowth.com/connect/zapier/choose-affiliate-program?state=abc123&redirect_uri=https://zapier.com/callback`
...
6. Zapier can now make authenticated API requests to Push Lap Growth
